...
process_diffs_to_wide_format <- function(d) {
  # Return empty table if input is NULL or has no rows
  if (is.null(d) || !nrow(d)) {
    return(data.frame(
  ...
  # ---- Blocking: treat one run of deletions followed by one run of insertions as a single block ----
  ops <- ifelse(startsWith(d$diff, "<"), "del", "add")
  block_id <- integer(nrow(d))
  i <- 1L; bid <- 0L
  while (i <= nrow(d)) {
    bid <- bid + 1L
    # consume a run of deletions
    while (i <= nrow(d) && ops[i] == "del") { block_id[i] <- bid; i <- i + 1L }
    # immediately consume the following run of insertions into the same block
    while (i <= nrow(d) && ops[i] == "add") { block_id[i] <- bid; i <- i + 1L }
  }
  d$block_id <- block_id
  
  # ---- Summarise: merge all A/B lines within each block; in replacement blocks force B to inherit A's line number ----
  out <- d %>%
    dplyr::group_by(block_id) %>%
    dplyr::summarise(
      hasA = any(startsWith(diff, "<")),
      hasB = any(startsWith(diff, ">")),
      # record the first available position in each side (used for pure insert/delete)
    ...
    # replacement blocks (hasA & hasB): force B side to inherit A's position; otherwise use its own
    dplyr::mutate(
